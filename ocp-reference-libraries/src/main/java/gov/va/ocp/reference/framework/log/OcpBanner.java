 ...
 * @author aburkholder
 */
public class OcpBanner {

	/** WARNING - DO NOT USE BANNER LOGGING FROM WITHIN THIS CLASS */
	private static final OcpLogger LOGGER = OcpLoggerFactory.getLogger(OcpBanner.class);
	 ...
	 */
	public OcpBanner(String bannerText, Level level) {
	...
	/**
	 * Convenience method. Same as {@link #OcpBanner(String, Level)}
	 *
	 * @param bannerText the banner text
	 * @param level the log level reported by the banner
	 * @return OcpBanner
	 */
	public static OcpBanner newBanner(final String bannerText, final Level level) {
		return new OcpBanner(bannerText, level);
	}
