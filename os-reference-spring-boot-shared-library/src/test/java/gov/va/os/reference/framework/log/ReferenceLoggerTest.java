...
import static gov.va.os.reference.framework.log.ReferenceBaseLogger.MAX_TOTAL_LOG_LEN;
...
public class ReferenceLoggerTest extends AbstractBaseLogTester {

	private ReferenceLogger logger = super.getLogger(ReferenceLoggerTest.class);

	private final ReferenceBanner banner = ReferenceBanner.newBanner("TEST BANNER", Level.INFO);
	private static final String MESSAGE = "Test message";
	private static final Marker MARKER = ReferenceLogMarkers.TEST.getMarker();
	private static final ReferenceRuntimeException EXCEPTION = new ReferenceRuntimeException("TestException");
	...
	public final void testLargeMessage() {
		...
		int mdcReserveLength = gov.va.os.reference.framework.log.ReferenceBaseLogger.MDC_RESERVE_LENGTH;

		int captureCount = 0;

		if ((mdcReserveLength + messageLength + stackTraceLength) > MAX_TOTAL_LOG_LEN) {
			if (messageLength >= gov.va.os.reference.framework.log.ReferenceBaseLogger.MAX_MSG_LENGTH) {
			...
			if ((stackTraceLength >= gov.va.os.reference.framework.log.ReferenceBaseLogger.MAX_STACK_TRACE_TEXT_LENGTH)) {
	...
	public final void testLargeMessageWithLargeStackTrace() {
		...
		Exception e = new ReferenceRuntimeException("TestException");
	...
	public final void testGetName() throws IOException {
		...
		assertNotNull(name);
		assertTrue(ReferenceLoggerTest.class.getName().equals(name));
	}
