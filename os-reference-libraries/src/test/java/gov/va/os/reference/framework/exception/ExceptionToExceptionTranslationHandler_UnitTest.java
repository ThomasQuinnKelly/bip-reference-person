...
import gov.va.os.reference.framework.exception.ExceptionToExceptionTranslationHandler;
import gov.va.os.reference.framework.service.ServiceException;
...
public class ExceptionToExceptionTranslationHandler_UnitTest {
	...
	public void handleViaTranslation_NoDefaultType() throws NoSuchMethodException, SecurityException{
		...
		ReferenceRuntimeException ascentRuntimeException = new ReferenceRuntimeException("ReferenceRuntimeException on purpose for unit test");
		try{
			handler.handleViaTranslation(getMethodToUseInTests(), null, ascentRuntimeException);
			Assert.fail("expected exception");
		} catch (Throwable throwable){
			Assert.assertTrue(throwable instanceof ReferenceRuntimeException);
