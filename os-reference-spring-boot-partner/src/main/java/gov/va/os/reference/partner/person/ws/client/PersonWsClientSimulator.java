...
import gov.va.os.reference.framework.ws.client.BaseWsClientSimulator;
...
public class PersonWsClientSimulator extends BaseWsClientSimulator implements PersonWsClient {
	...
	private JAXBElement<FindPersonBySSNResponse> getSimulatedResponse() {
		...
		} catch (final JAXBException e) { // NOSONAR exception is logged
		...
		try {
			if (jc != null) {
				...
				responseElement = unmarshaller.unmarshal(getXMLStreamReader(personWSResponseFile),
	 ...
	 */
	private XMLStreamReader getXMLStreamReader(final String personWSResponseFile) {
		...
		} catch (final XMLStreamException e) { // NOSONAR exception is logged
